...
export class PragmaticFastMessaging {
  ...
  async sendMessage(text, replyTo = null, providedId = null) {
    ...
    try {
      // If socket is available, use it for low-latency send with ack
      if (this.socket && this.socket.connected) {
        const ack = await new Promise((resolve, reject) => {
          ...
          // fallback timeout
          setTimeout(() => reject(new Error('Socket ack timeout')), 5000);
        });
      ...
      if (response.ok) {
        ...
        this.updateLatencyStats(latency);
        this.latencyStartTimes.delete(messageId);
        if (this.pendingMessages.has(messageId)) this.pendingMessages.delete(messageId);
