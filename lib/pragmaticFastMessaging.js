...
export class PragmaticFastMessaging {
  constructor(username, jwtToken, config = {}) {
    ...
    this.config = {
      room: HELLOW_ROOM,
      maxRetries: 3,
  heartbeatInterval: 20000, // more frequent for fresher presence
  presenceStaleMs: 90000, // mark offline if no heartbeat in 90s
  presenceRecentMs: 5 * 60 * 1000, // recently active threshold
  presenceAwayMs: 30 * 60 * 1000, // away threshold
  typingDebounce: 450,
  typingIntentChars: 3,
  typingTimeout: 3200,
  ...
  this.receiptHandlers = new Set();
  // deliveryState: messageId -> { readers: { username: timestamp }, optimistic?: boolean }
  this.deliveryState = new Map();
  this.presenceState = { peers: {}, lastSync: 0 };
  this.heartbeatTimer = null;
  this.presenceSyncTimer = null;
  this.deviceType = detectDeviceType();
  this.lastSentTyping = 0;
  this.typingDebounceTimer = null;
  this.currentTypingCategory = 'short';
  // Read receipt batching & reconciliation
  this.pendingReadIds = new Set();
  this.readBatchTimer = null;
  this.readsSyncDone = {}; // target -> true
  this._earlyReceiptBuffer = new Map(); // messageId -> [{reader, timestamp}]
  this._pendingReadsPersisted = false;
      ...
      this._idbPromise = openDB('pragmatic-fast-messaging-db', 1, {
        upgrade(db) {
          ...
          if (!db.objectStoreNames.contains('pendingReads')) {
            db.createObjectStore('pendingReads', { keyPath: 'id' });
          }
        },
      });
  ...
  async idbAddPendingReads(ids) {
    try {
      if (!ids?.length) return;
      const db = await this.openIdb();
      const tx = db.transaction('pendingReads', 'readwrite');
      for (const id of ids) {
        await tx.store.put({ id, createdAt: Date.now() });
      }
      await tx.done;
    } catch (e) { console.warn('idbAddPendingReads failed', e); }
  }

  async idbGetPendingReads() {
    try { const db = await this.openIdb(); return (await db.getAll('pendingReads')) || []; } catch (e) { return []; }
  }

  async idbRemovePendingReads(ids) {
    try { if (!ids?.length) return; const db = await this.openIdb(); const tx = db.transaction('pendingReads','readwrite'); for (const id of ids) await tx.store.delete(id); await tx.done; } catch (e) {}
  }
  ...
  // Start heartbeat & presence reconciliation
  this.startHeartbeat();
  this.startPresenceSyncLoop();
    
  // Load any persisted queued messages from storage and process them
  await this.loadQueueFromStorage();
  // Sync previously stored reads
  this.syncHistoricalReads();
  // Load persisted pending read receipts & schedule flush
  try {
    const pendingReads = await this.idbGetPendingReads();
    if (pendingReads.length) {
      pendingReads.forEach(r => this.pendingReadIds.add(r.id));
      if (!this.readBatchTimer) this.readBatchTimer = setTimeout(() => this.flushReadBatch(), 400);
    }
  } catch (e) {}
  ...
  // Resolve external socket base URL if provided (enables cross-origin)
  const rawEnvUrl = (typeof window !== 'undefined' && window?.__NEXT_DATA__?.env?.NEXT_PUBLIC_SOCKET_URL) || process.env.NEXT_PUBLIC_SOCKET_URL || '';
  const socketUrl = rawEnvUrl ? rawEnvUrl.replace(/\/$/, '') : '';
  // Force websocket transport when crossing origins to avoid extra polling CORS preflights / 502s
  const baseConnectOpts = {
    auth: { token: this.jwtToken },
    path: '/api/socketio',
    transports: socketUrl ? ['websocket'] : undefined, // only force when remote; local can negotiate
    withCredentials: true,
    reconnectionAttempts: 3,
  };
  const connectOpts = baseConnectOpts;
  // Track repeated polling 404s (Render often returns 404 for polling path when not provisioned)
  this._polling404s = this._polling404s || 0;
  if (socketUrl) {
    try { console.log('[socket] attempting remote connect ->', socketUrl, connectOpts); } catch (_) {}
    this.socket = io(socketUrl, connectOpts);
  } else {
    try { console.log('[socket] attempting same-origin connect', connectOpts); } catch (_) {}
    this.socket = io('/api/socketio', connectOpts);
  }
  ...
  // NEW: receipt events
  this.socket.on('receipt', (data) => this.handleIncomingEvent({ ...data, type: 'receipt' }));
      ...
      this.socket.io?.engine?.on?.('upgradeError', (err) => {
        console.warn('[socket] upgradeError', err?.message || err);
      });
      this.socket.io?.engine?.on?.('close', (reason) => {
        if (/transport error/i.test(reason || '')) console.warn('[socket] engine close due to transport error');
      });
      this.socket.io?.on?.('reconnect_failed', () => {
        console.warn('[socket] reconnect_failed -> switching to SSE');
        try { this.socket?.close(); } catch(_) {}
        this.socket = null;
        this.connectSSE();
      });

      // Detailed diagnostics for CORS / handshake / transport failures
      this.socket.on('connect_error', (err) => {
        try {
          ...
          // Fast fallback if we detect typical CORS / 404 on polling path
          if (/cors|xhr poll|404|502/i.test(err?.message || '')) {
            console.warn('↩️ Falling back to SSE due to connect_error heuristic');
            try { this.socket?.close(); } catch (_) {}
            this.socket = null;
            this.connectSSE();
          }
  ...
  connectSSE() {
    try {
      ...
      // NEW: receipt events for SSE
      this.eventSource.addEventListener('receipt', (event) => {
        try {
          const payload = JSON.parse(event.data);
          this.handleReceiptEvent(payload);
        } catch (e) {
          console.warn('Failed to parse receipt event', e);
        }
      });
  ...
  sendTyping(isTyping = true) {
    ...
    // (Could be expanded to analyze current input length passed externally if we refactor)
    // For now we just cycle categories heuristically
    const now = Date.now();
    const sinceLast = now - this.lastSentTyping;
    if (sinceLast > 5000) this.currentTypingCategory = 'long';

    clearTimeout(this.typingDebounceTimer);
    this.typingDebounceTimer = setTimeout(() => {
      this.sendTypingUpdate(true, { category: this.currentTypingCategory });
      this.lastSentTyping = Date.now();
      clearTimeout(this.typingTimeout);
      this.typingTimeout = setTimeout(() => this.sendTypingUpdate(false), this.config.typingTimeout);
    }, this.config.typingDebounce);
  }

  async sendTypingUpdate(isTyping, meta = {}) {
    try {
  ...
  this.recordAckLatency(latency);
    ...
    // Reconcile early buffered receipts for this message
    if (this._earlyReceiptBuffer && this._earlyReceiptBuffer.has(data.id)) {
      const buffered = this._earlyReceiptBuffer.get(data.id) || [];
      const entry = this.deliveryState.get(data.id) || { readers: {} };
      entry.readers = entry.readers || {};
      buffered.forEach(r => { if (!entry.readers[r.reader]) entry.readers[r.reader] = r.timestamp; });
      this.deliveryState.set(data.id, entry);
      this._earlyReceiptBuffer.delete(data.id);
    }
    this.messageHandlers.forEach(handler => handler({ ...data, isPending: false, isFailed: false }));
  }
  ...
  handleReceiptEvent(data) {
    try {
      const { messageId, reader, timestamp } = data;
      if (!messageId || !reader) return;
      let entry = this.deliveryState.get(messageId);
      if (!entry) {
        // Buffer early receipt until message arrives
        if (this._earlyReceiptBuffer) {
          const arr = this._earlyReceiptBuffer.get(messageId) || [];
          arr.push({ reader, timestamp });
          this._earlyReceiptBuffer.set(messageId, arr);
        }
        return;
      }
      entry.readers = entry.readers || {};
      if (!entry.readers[reader]) entry.readers[reader] = timestamp;
      if (entry.optimistic && reader === this.username) entry.optimistic = false;
      this.deliveryState.set(messageId, entry);
      this.receiptHandlers.forEach(h => h({ messageId, readers: entry.readers, reader }));
    } catch (e) {
      console.warn('Failed to process receipt event', e);
    }
  }
  ...
  const id = message.id;
  const existing = this.deliveryState.get(id) || { readers: {} };
  if (existing.readers?.[this.username]) return; // already marked locally
  existing.readers[this.username] = Date.now();
  existing.optimistic = true; // until server confirms
  this.deliveryState.set(id, existing);
  this.pendingReadIds.add(id);
  // Persist pending read for offline durability
  this.idbAddPendingReads([id]);
  if (!this.readBatchTimer) this.readBatchTimer = setTimeout(() => this.flushReadBatch(), 160);
    } catch (e) {}
  }

  async flushReadBatch() {
    ...
    try {
      await fetch('/api/fast-chat', {
        ...
        body: JSON.stringify({
          action: 'receipt_batch',
          room: this.config.room,
          username: this.username,
          messageIds: ids
        })
      });
  // Mark optimistic reads as confirmed & remove from persisted pending store
  ids.forEach(id => { const entry = this.deliveryState.get(id); if (entry) entry.optimistic = false; });
  this.idbRemovePendingReads(ids);
    } catch (e) {
      // If failed, requeue once
      ids.forEach(id => this.pendingReadIds.add(id));
      if (!this.readBatchTimer) this.readBatchTimer = setTimeout(() => this.flushReadBatch(), 2000);
    }
  }

  async syncHistoricalReads(targetPeer = null) {
    const target = targetPeer || this.username;
    if (this.readsSyncDone[target]) return;
    try {
      const res = await fetch('/api/fast-chat', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${this.jwtToken}` },
        body: JSON.stringify({ action: 'reads_sync', room: this.config.room, username: this.username, peer: targetPeer })
      });
      if (!res.ok) return;
      const data = await res.json();
      if (data.success && Array.isArray(data.reads)) {
        data.reads.forEach(id => {
          const entry = this.deliveryState.get(id) || { readers: {} };
          entry.readers = entry.readers || {};
          if (!entry.readers[target]) entry.readers[target] = Date.now();
          this.deliveryState.set(id, entry);
        });
      }
      this.readsSyncDone[target] = true;
    } catch (e) {}
  ...
  startPresenceSyncLoop() {
    if (this.presenceSyncTimer) clearInterval(this.presenceSyncTimer);
    this.presenceSyncTimer = setInterval(() => this.syncPresenceState(), 60000);
    // initial sync
    this.syncPresenceState();
  }

  async syncPresenceState() {
    try {
      const res = await fetch('/api/fast-chat', {
        method: 'POST',
        headers: {
      ...
      if (data.success && data.presence) {
        this.presenceState.peers = data.presence;
        this.presenceState.lastSync = Date.now();
        // Emit synthetic presence events for each peer
        Object.entries(data.presence).forEach(([peer, info]) => {
          if (peer === this.username) return;
...
function detectDeviceType() {
  ...
  if (/mobile|iphone|android|ipad|ipod/.test(ua)) return 'mobile';
