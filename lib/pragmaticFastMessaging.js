...
export class PragmaticFastMessaging {
  ...
  persistQueueToStorage() {
    try {
      ...
      window.localStorage.setItem(this.getQueueStorageKey(), JSON.stringify(toStore));
    } catch (e) {
  ...
  loadQueueFromStorage() {
    try {
      ...
      // Rehydrate into messageQueue; these entries have no promises attached
      for (const item of arr) {
        const entry = { message: item.message, resolve: null, reject: null, createdAt: item.createdAt || Date.now(), fallbackTimer: null };
        // Start a fallback timer so these persisted entries will use HTTP after timeout
        entry.fallbackTimer = setTimeout(async () => {
          try {
            ...
            this.removeFromStorageById(entry.message.id);
            // resolve if promise exists (shouldn't for persisted entries)
  ...
  removeFromStorageById(id) {
    try {
      ...
      if (!raw) return;
      const arr = JSON.parse(raw || '[]');
      const filtered = arr.filter(item => item.message && item.message.id !== id);
      window.localStorage.setItem(key, JSON.stringify(filtered));
    } catch (e) {
      console.warn('Failed to remove queued message from storage:', e);
    }
  }
    ...
    return new Promise((resolve, reject) => {
      ...
      entry.fallbackTimer = setTimeout(async () => {
        try {
          ...
          // Remove from persisted storage
          this.removeFromStorageById(message.id);
