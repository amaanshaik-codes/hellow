...
export class PragmaticFastMessaging {
  ...
  // IndexedDB helpers (minimal wrapper)
  openIdb() {
    if (this._idbPromise) return this._idbPromise;
    this._idbPromise = new Promise((resolve, reject) => {
      try {
        const req = indexedDB.open('pragmatic-fast-messaging-db', 1);
  ...
  async idbPutQueue(arr) {
    try {
      ...
      // Replace by clearing and adding
      const clearReq = store.clear();
      await new Promise((res, rej) => { clearReq.onsuccess = res; clearReq.onerror = rej; });
      for (const item of arr) {
        const putReq = store.put({ id: item.message.id, createdAt: item.createdAt || Date.now(), message: item.message });
        await new Promise((res, rej) => { putReq.onsuccess = res; putReq.onerror = rej; });
      }
  ...
  async idbGetQueue() {
    try {
      const db = await this.openIdb();
      const tx = db.transaction('queuedMessages', 'readonly');
      const store = tx.objectStore('queuedMessages');
