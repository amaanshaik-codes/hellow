  ...
  async sendMessage(text, replyTo = null, providedId = null) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Use server-provided storedTimestamp as the canonical persisted time
        ...
        // Clear pending state for this message and mark as delivered
        if (this.pendingMessages.has(messageId)) {
          this.pendingMessages.delete(messageId);
        }

        // Add to seen messages to prevent duplicate UI insertion
        try {
          this.seenMessageIds.add(messageId);
        } catch (e) {}

        console.log(`📤 Message sent and persisted (latency ${latency}ms)`);
        return { ...result.message, timestamp: storedTs };
      } else {
