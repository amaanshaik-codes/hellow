...
export class PragmaticFastMessaging {
  ...
  reconnectSSE() {
    if (this.retryCount >= this.config.maxRetries) {
      ...
      // Reset retry count after 5 minutes to allow reconnection
      setTimeout(() => {
        console.log('🔄 Resetting retry count, allowing reconnection...');
        this.retryCount = 0;
      }, 300000);
      return;
    }

    const delay = Math.min(this.config.reconnectDelay * Math.pow(1.5, this.retryCount), 30000);
