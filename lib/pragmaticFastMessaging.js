...
export class PragmaticFastMessaging {
  ...
  this.deliveryState = new Map(); // messageId -> { delivered: ts, read: ts }
  ...
  handleReceiptEvent(data) {
    try {
      ...
      if (!messageId) return;
      // Update delivery/read map
      const existing = this.deliveryState.get(messageId) || {};
      if (reader !== this.username) {
        // Treat as read receipt from peer
        existing.read = existing.read || timestamp;
      }
      this.deliveryState.set(messageId, existing);
      this.receiptHandlers.forEach(h => h({ ...existing, messageId, reader }));
    } catch (e) {
  ...
  async sendReadReceipt(message) {
    try {
      ...
      const id = message.id;
      const existing = this.deliveryState.get(id) || {};
      if (existing.read) return; // already marked
      existing.read = Date.now();
      this.deliveryState.set(id, existing);
      this.pendingReadIds.add(id);
      if (!this.readBatchTimer) {
        this.readBatchTimer = setTimeout(() => this.flushReadBatch(), 160);
      }
    } catch (e) {}
  }
  ...
  async syncHistoricalReads() {
    if (this.readsSyncDone) return;
    try {
      const res = await fetch('/api/fast-chat', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${this.jwtToken}` },
        body: JSON.stringify({ action: 'reads_sync', room: this.config.room, username: this.username })
      });
      if (!res.ok) return;
      const data = await res.json();
      if (data.success && Array.isArray(data.reads)) {
        data.reads.forEach(id => {
          const entry = this.deliveryState.get(id) || {};
          entry.read = entry.read || Date.now();
          this.deliveryState.set(id, entry);
        });
      }
      this.readsSyncDone = true;
    } catch (e) {}
  }
